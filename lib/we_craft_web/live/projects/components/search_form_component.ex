defmodule WeCraftWeb.Projects.Components.SearchFormComponent do
  ...
  defp filter_tags(tags, filter) when is_binary(filter) and filter != "" do
  ...
  defp filter_tags(tags, _), do: tags

  # Helper function to compare status values correctly
  defp selected_status?(nil, ""), do: true
  defp selected_status?(status, value) when is_atom(status), do: Atom.to_string(status) == value
  defp selected_status?(_, _), do: false
end
