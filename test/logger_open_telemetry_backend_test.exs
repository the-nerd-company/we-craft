defmodule WeCraft.LoggerOpenTelemetryBackendTest do
  ...
  test "handles errors gracefully" do
    # We'll just verify that the module exports all the required callbacks
    # for the :gen_event behavior and doesn't crash on basic operations

    assert function_exported?(LoggerOpenTelemetryBackend, :init, 1)
    assert function_exported?(LoggerOpenTelemetryBackend, :handle_event, 2)
    assert function_exported?(LoggerOpenTelemetryBackend, :handle_call, 2)
    assert function_exported?(LoggerOpenTelemetryBackend, :handle_info, 2)
    assert function_exported?(LoggerOpenTelemetryBackend, :terminate, 2)

    # Test that the module doesn't crash on various operations
